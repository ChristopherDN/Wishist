...
public class TempProductService {
  ...
  public ArrayList<TempProduct> getProductsWishlist(User user, String event) {
    for (int i = 0; i < user.getWishlist().size(); i++) {
      if (user.getWishlist().get(i).getEvent().equals(event)) {
        ...
        return sqlProducts.getProductsWishlist(wishlistId);
      }
    }
    return sqlProducts.getProductsWishlist(0);
  }
}
